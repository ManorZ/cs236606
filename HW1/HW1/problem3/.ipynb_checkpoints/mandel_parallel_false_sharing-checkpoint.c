...
int main() {
   ...
   for (int k=0; k< NELEMS(threads); k++) {
       ...
       for (int kk = 0 ; kk < num_of_threads ; kk++)
            numoutside += numoutside_per_thread[kk];
       ...
       printf("Parallel-False-Sharing implementation - time: %f seconds, with %d threads\n\n",run_time, num_of_threads);
   }
...
void testpoint(int thread_id) {
   ...
   for (iter = 0; iter < MAXITER; iter++) {
      ...
      if ((z.r * z.r + z.i * z.i) > 4.0) {
         //numoutside++;
         numoutside_per_thread[thread_id]++;
         break;
      }
   }
}
